...
public:
	Solver(const FixedField& fixedField, const VariableField& variableField, const Match& match, const Teams& teams) :
...
private:
	...
	Action getAction(const Agent& agent) {
		...
		while (get<0>(_deque.first.front()).turn < min(1 + _match.turns, 16 + _variableField.turn)) {
			while (!_deque.first.empty()) {
				...
				switch (get<3>(_deque.first.front()).first.type) {
